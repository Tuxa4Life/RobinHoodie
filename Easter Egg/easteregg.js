  ...
  class ProgressBar {
    ...
    detectGameEnd() {
      ...
      if (this.progress >= 100) {
        let card = document.querySelector('#won')
        card.style.display = 'flex'

        let code = userValues.UID + localStorage.getItem('FISH')[0].toUpperCase()
        card.children[1].textContent = code

        card.children[2].addEventListener ('click', () => {
          if (navigator && navigator.clipboard && navigator.clipboard.writeText) {
            code = userValues.UID + localStorage.getItem('FISH')[0].toUpperCase()
            navigator.clipboard.writeText(code)
            card.children[2].textContent = 'დაკოპირებულია'
          } else {
            alert(`Error: Browser Doesn't Support Clipboard API`)
            card.children[2].textContent = 'Error'
          }
        })
